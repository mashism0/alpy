...
async def ask_for_qr_photo(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer("📸 Пожалуйста, отправьте фото QR-кода с удостоверения инструктора.")  # Запросить фото у пользователя
...
async def handle_qr_photo(message: Message, state: FSMContext):
    ...
    try:
        ...
        if image is None:
            await message.answer("❌ Не удалось загрузить изображение.")  # Если не получилось прочитать - сообщаем об ошибке
            return
        ...
        if not qr_data:
            await message.answer("❌ Не удалось распознать QR-код.")  # Если QR-код не распознан - сообщаем
            return

        if not qr_data.startswith("http"):  # Проверяем, что QR содержит ссылку
            await message.answer("❌ QR-код не содержит ссылку.")
            return

        await message.answer("🔍 Ищу информацию на сайте...")  # Информируем пользователя о начале парсинга
...
def fetch_info_selenium_sync(url: str) -> dict | None:
    ...
    try:
        ...
        return {"full_name": full_name, "city": city, "status": status}  # Возвращаем словарь с данными
